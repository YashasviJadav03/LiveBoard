...
TEST_DATABASE_URL = "sqlite+aiosqlite:///./test_phase2.db"
...
async def setup_database():
    """Create tables before each test, drop after. Also flush Redis test keys."""
    ...
    try:
        keys = await redis_client.keys("lb:test_*")
        if keys:
